...
class ToastContainer extends Component<Props, State> {
  ...
  private listenerSubscriptions: EmitterSubscription[] = [];
  ...
  componentWillUnmount = () => {
    this._hide();
    this.listenerSubscriptions.forEach(sub => sub.remove());
    this.listenerSubscriptions = [];
  };

  _windowChanged = ({window}: {window: {width: number; height: number}}) => {
    this.setState({
      windowWidth: window.width,
      windowHeight: window.height,
    });
